...
public class ZoomManager {
    ...
    public static void tick(MinecraftClient client, boolean isKeyDown) {
        ...
        targetBarsPct = zoomHeld ? (float) (config.barsPercent / 100.0) : 0.0f;
    }
    ...
    public static void frameUpdate() {
        ...
        // Fast black bars logic - we want them to appear quickly when zooming
        // but can use smooth interpolation.
    ...
    public static boolean onWheel(double vertical) {
        ModConfig config = com.ji.zoomcinematic.config.ConfigManager.getConfig();
        if (!zoomHeld || !config.mouseWheelEnabled) {
            return false;
        }
        if (vertical == 0.0) {
            return false;
        }
        float step = 0.05f; // Hardcoded step for simplicity, similar to original
        if (vertical > 0.0) {
            holdZoomMul -= step;
        } else {
            holdZoomMul += step;
        }
        holdZoomMul = clamp(holdZoomMul, 0.05f, 1.0f); // Max zoom allows up to 0.05x FOV
        return true;
    }
